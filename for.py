...
def forward(board, col):
    ...
    if col >= N:
        ...
        printrows(board)
        bkls.append(bk)
        print("Number of backtracks for forward checking this solution: ",bk)#print back tracks
        bk=0
        return True
    ...
    for i in range(N):#loop through columns
        ...
        if checkconsit(board, i, col):
            ...
            forward(board, col+1)#recursive call to check the next availaile squares
...
def main():#init board
    ...
    #print(bk)
    totalbk=sum(bkls)+bk
    print("Total number of forward checking backtracks:",totalbk)
    return True
